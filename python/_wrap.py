...
import _etagen
from numpy import array, hstack, vstack, loadtxt, median
from matplotlib.pyplot import scatter

class etagen(_etagen.etagen):
	# dtype for trigger informations
	ptrg_dtype = [('imf_idx',int), ('s_time',float), ('e_time',float), ('p_time',float), ('p_amp',float), ('p_freq',float), ('f_min',float), ('f_max',float), ('snr',float)]
	...
	def get_ptriggers(self, snr_th=3, stride=0, overlap=0, skip=0):
		"""
		self.get_ptriggers(snr_th=3, stride=0, overlap=0, skip=0):

		Generate triggers, provided that Intrinsic Mode Functions(IMFs) are
		obtained before self.get_ptriggers() is called.
		...
		Output:
			...
			self.ptrgs	The trigger information generated.
					The information is [IMF index, start time, end time,
					peak time, peak amplitude, peak frequency, snr].
		"""
		if self.data is None:
			raise RuntimeError("data should be loaded before get_ptriggers()")
		...
		print "Generating triggers with {0}-p_snr threshold in segments of length {1}, overlapping {2} samples and skipping {3} samples from boundaries...".format(snr_th, stride, overlap, skip)
		# imf_idx, s_time, e_time, p_time, p_amp, p_freq, f_min, f_max, snr
		self.ptrgs = array([], dtype=etagen.ptrg_dtype)#.reshape(0,1)
		for j in range(skip, self.data_len-skip-overlap, stride-overlap):
			_trgs = self.gen_ptrgs(self.min_snr, j, stride)
			...
			self.ptrgs = hstack((self.ptrgs, array(zip(_trgs[:,0].astype(int), _trgs[:,1], _trgs[:,2], _trgs[:,3], _trgs[:,4], _trgs[:,5], _trgs[:,6], _trgs[:,7], _trgs[:,8]), dtype=etagen.ptrg_dtype)))
		self.ptrgs.sort(order=['imf_idx', 'p_time'])
		print "... generated %i trigger event(s)" % (self.ptrgs.shape[0])

	def get_ptriggers_from_file(self, trg_file, **kwargs):
		"""
		self.get_ptriggers_from_file(trg_file, **kwargs):
		...
		"""
		# imf_idx, s_time, e_time, p_time, p_amp, p_freq, f_min, f_max, snr
		self.ptrgs = loadtxt(trg_file, dtype=etagen.ptrg_dtype, **kwargs)
		self.min_snr = int(self.ptrgs['snr'].min())
		print "... read %i trigger event(s)" % (self.ptrgs.shape[0])

	def get_triggers(self, t_tolerance=0., f_tolerance=0., p_snr_th=3, snr_th=10):
		"""
		self.get_triggers(maxDist=2):

		Cluster trigger events generated by self.get_ptriggers().

		Required item:

			self.ptrgs	triggers generated by self.get_ptriggers()

		Input parameters:

			maxDist		maximum distance to cluster tiggers. Default is 2.
			unit_time	unit time for distance. Default is 0.01(=10ms).
			p_snr_th	A threshold parameter for each trigger event. Default is 3.
			snr_th	A threshold parameter for a cluster of triggers.
					Default is 10.
	
		Output:

			self.ptrgs	The trigger information generated if not calculated before.
					The information is [IMF index, start time, end time,
					peak time, peak amplitude, peak frequency, snr].
		...
		if self.ptrgs is None:
			self.get_ptriggers(snr_th=p_snr_th)
		if p_snr_th < self.min_snr:
			print "p_snr_th should be larger than the one used to generate triggers: assuming p_snr_th =", self.min_snr
			self.p_snr_threshold = self.min_snr
		else:
			self.p_snr_threshold = p_snr_th
		self.snr_threshold = snr_th
		#self.waveforms = []
		print "Clustering triggers of p_snr > {0} with time tolerance={1}, frequency tolerance={2} and dropping clusters of snr < {3}...".format(self.p_snr_threshold, t_tolerance, f_tolerance, snr_th)
		# s_time, e_time, c_time, c_freq, c_energy, p_time, p_freq, p_amp, p_imf_idx, p_snr, npts, snr

		carg = self.ptrgs['snr']>=self.p_snr_threshold
		ptrgs = self.ptrgs[carg]

		_trgs = self.gen_trgs(ptrgs, snr_threshold=self.snr_threshold, t_tolerance=t_tolerance, f_tolerance=f_tolerance)
	...
	def copy(self, start=None, end=None):
		...
		*start*/*end* should be greater than self.start_time
		...
		if self.ptrgs is not None:
			hhtout.ptrgs  = self.ptrgs[(self.ptrgs['p_time'] > hhtout.start_time) & (self.ptrgs['p_time'] < end_time)]
			hhtout.ptrgs[hhtout.ptrgs['s_time'] < hhtout.start_time]['s_time'] = hhtout.start_time
			hhtout.ptrgs[hhtout.ptrgs['e_time'] > end_time]['e_time'] = end_time
		...
		if self.trgs is not None:
			...
			try:
				hhtout.p_snr_threshold = self.p_snr_threshold
			except:
				pass
